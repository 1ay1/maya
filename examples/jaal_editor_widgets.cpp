// examples/jaal_editor_widgets.cpp — jaal port of examples/editor_widgets.cpp (view unchanged).
//
...
struct Show {
    ...
    static Cmd update(Model& m, Down) { m.k = m.k + 1; return {}; }
...
static_assert(JaalView<Show>);

int main() { return run_jaal<Show>({.title = "maya editor widgets"}); }
