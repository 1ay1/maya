...
Element ChatView::build() {
    ...
    // Messages area — rendered directly, no virtual scroll.
    // Terminal scrollback provides native scrolling in inline mode.
    Element messages_elem = [&]() -> Element {
        if (messages_.empty()) {
    ...
    layout.reserve(6);
    layout.push_back(std::move(messages_elem));
