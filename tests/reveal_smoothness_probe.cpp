...
int main() {
    // Freeze the animation clock: with only the SKEW (advance_anim_clock_ms)
    // on top of a LIVE steady_clock, real wall-time between two synchronous
    // build() calls leaks into the reveal cursor's dt — under machine load a
    // slow frame integrates extra time and reveals more cells, flaking the
    // per-frame cap (observed: PASS on one run, +33 > cap on the next, no
    // code change). Frozen, the probe is a pure function of the feed.
    ...
    for (std::size_t i = 0; i < stats.size(); ++i) {
        ...
        const char* name = sec < static_cast<int>(doc.size())
            ? doc[static_cast<std::size_t>(sec)].name : "drain";
        // Eager blocks (table / list / quote / CODE) render their rows via
        // lazy width-aware components and reveal ROW-granular — a completed
        // row legitimately appears as a unit, so their per-frame delta is
        // bounded by a row's width (~one wrapped line), not a single cell.
        // They get a higher cap. Prose / heading reveal per-glyph and stay
        // strict. Code was misclassified as strict until the anim clock was
        // frozen: the wall-clock leak jittered WHERE the row boundary
        // crossing landed, so the one-code-row pop (+33 ≈ one source line)
        // only sometimes hit a measured frame — the probe was FLAKY-green.
        // Deterministic now, and the row-pop is the DOCUMENTED line_bounded
        // behavior (maya 0a40613): per-glyph reveal is confined to the leaf's
        // bottom visual row because upper rows can't be proven outside
        // immutable scrollback; a completed row above the cursor appears
        // whole. Bounded by row width, cosmetic — same contract as table
        // rows. (Making eager blocks reveal per-cell across all rows was
        // TRIED and reverted: it re-rendered a row already in native
        // scrollback — reveal_scrollback_test R5 caught the duplicate.)
        // The strict cap still catches the old whole-BLOCK pop (+100..+285).
        const bool eager = name && (std::string_view{name} == "table"
                                 || std::string_view{name} == "list"
                                 || std::string_view{name} == "quote"
                                 || std::string_view{name} == "code");
