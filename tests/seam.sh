...
# include/maya/jaal/ is the seam. The rule that makes "maya is to jaal what
# Ink is to React" a fact instead of a slogan:
#
#   NOTHING outside include/maya/jaal/ may depend on jaal.
#
...
for f in $(grep -rl 'jaal' include/maya --include='*.hpp' 2>/dev/null); do
    case "$f" in
        include/maya/jaal/*) continue ;;      # the seam itself
        include/maya/app.hpp) continue ;;     # the seam's table of contents
    esac
...
if [ -n "$leaks" ]; then
    ...
    echo "  ok    only maya/jaal/ depends on jaal"
fi

# 3. The seam is where we say it is.
for f in interop effects sources host run; do
    [ -f "include/maya/jaal/$f.hpp" ] || fail "missing include/maya/jaal/$f.hpp"
done
