...
TEST_CASE("framebuffer first render nonempty") {
    ...
    FrameBuffer fb(30, 5);
    const auto& out = fb.render(text("hello"), theme::native);
    assert(!out.empty());
...
TEST_CASE("framebuffer contains sync markers") {
    ...
    FrameBuffer fb(20, 5);
    const auto& out = fb.render(text("test"), theme::native);
...
TEST_CASE("framebuffer same content second render smaller") {
    ...
    FrameBuffer fb(30, 5);
    const auto& out1 = fb.render(text("hello world"), theme::native);
    std::size_t size1 = out1.size();

    const auto& out2 = fb.render(text("hello world"), theme::native);
...
TEST_CASE("framebuffer changed content produces output") {
    ...
    FrameBuffer fb(30, 5);
    (void)fb.render(text("frame1"), theme::native);

    const auto& out = fb.render(text("frame2"), theme::native);
    assert(!out.empty());
...
TEST_CASE("framebuffer resize forces repaint") {
    ...
    FrameBuffer fb(20, 5);
    (void)fb.render(text("hello"), theme::native); // warm up
...
TEST_CASE("framebuffer swap front back") {
    ...
    FrameBuffer fb(20, 5);
    render_tree(text("front"), fb.front().canvas, fb.style_pool(), theme::native);
    render_tree(text("back"),  fb.back().canvas,  fb.style_pool(), theme::native);

    // After render(), the back becomes front
    (void)fb.render(text("new_back"), theme::native);
