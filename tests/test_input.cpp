...
static const KeyEvent& as_key(const Event& ev) {
    const auto* ke = std::get_if<KeyEvent>(&ev);
    assert(ke != nullptr);
    return *ke;
}

static const MouseEvent& as_mouse(const Event& ev) {
...
void test_input_ascii_char() {
    ...
    const KeyEvent& ke = as_key(events[0]);
...
void test_input_multiple_ascii_chars() {
    ...
    assert(std::get<CharKey>(as_key(events[0]).key).codepoint == U'x');
    assert(std::get<CharKey>(as_key(events[1]).key).codepoint == U'y');
    assert(std::get<CharKey>(as_key(events[2]).key).codepoint == U'z');
    std::println("PASS\n");
}
...
void test_input_right_arrow() {
    ...
    assert(!events.empty());
    const auto* sk = std::get_if<SpecialKey>(&as_key(events[0]).key);
...
void test_input_utf8_two_byte() {
    ...
    assert(!events.empty());
    const auto* ck = std::get_if<CharKey>(&as_key(events[0]).key);
...
void test_input_mouse_left_press() {
    ...
    assert(!events.empty());
    const MouseEvent& me = as_mouse(events[0]);
...
void test_input_mixed_sequence() {
    ...
    assert(events.size() == 3);
    assert(std::get<CharKey>(as_key(events[0]).key).codepoint == U'a');
    assert(*std::get_if<SpecialKey>(&as_key(events[1]).key) == SpecialKey::Right);
    assert(std::get<CharKey>(as_key(events[2]).key).codepoint == U'b');
    std::println("PASS\n");
}
